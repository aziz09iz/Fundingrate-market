...
/**
 * The current wall clock, for countdowns.
 *
 * Passed through context rather than as a prop so a tick does not change the props of
 * every row. Countdowns need a re-render every second; the rest of a row does not, and
 * at 250 rows the difference is the whole table re-rendering twice a second against
 * only the cells that show a clock.
 */
...
interface FundingTableProps {
  /** One page, already filtered, re-scoped and sorted by the server. */
  rows: MarketViewRow[];
  ...
  /**
   * True until the first frame arrives. Skeleton rows say "connecting", where
...
export function FundingTable({
  ...
  return (
    <NowContext.Provider value={nowMs}>
 ...
 * One coin's row.
 *
 * Memoised on the row object, which the server rebuilds only for coins whose readings
 * changed — so a 1 Hz frame re-renders the rows that moved rather than the whole page.
 * The clock arrives through context, so a countdown tick does not defeat this.
 */
 ...
 *
 * Split out so the clock tick reaches only the text that shows a clock. It sits inside
 * a tooltip that is mounted on hover, so at a hundred rows there is normally exactly
 * one of these subscribed to the tick rather than a hundred.
 */
function Countdown({ target }: { target: number }) {
  const nowMs = useNow();
  return <> · next in {formatCountdown(target, nowMs)}</>;
}
...
function DiffCell({ value }: { value: number | null }) {
  const { direction, onAnimationEnd } = useValueFlash(value);
  return (
    <TableCell
      data-flash={direction ?? undefined}
...
function RateCell({
  ...
  return (
    <HoverTooltip
      className="flex cursor-help flex-col items-end leading-tight"
      contentClassName="font-mono text-xs"
      content={
        <div className="flex flex-col gap-0.5">
          <span>
            {intervalHours}h interval{intervalConfirmed ? "" : " (assumed)"}
            {nextFundingTime > 0 && <Countdown target={nextFundingTime} />}
          </span>
          <span>
 ...
 */
function SpreadCell({ row }: { row: MarketViewRow }) {
  const spread = row.priceSpread;
  const { direction, onAnimationEnd } = useValueFlash(spread?.pct ?? null);
  if (!spread) return <span className="text-muted-foreground/60">—</span>;
  return (
    <span data-flash={direction ?? undefined} onAnimationEnd={onAnimationEnd}>
      <HoverTooltip
...
function tradeHref(row: MarketViewRow, mode: "instant" | "delay"): string {
